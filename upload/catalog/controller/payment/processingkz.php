...
class ControllerPaymentProcessingkz extends Controller {
    ...
    public function index() {
        ...
        $data['action'] = HTTPS_SERVER . 'index.php?route=payment/processingkz/processing';
		...
		if (version_compare(VERSION, '2.2.0', '>=')) {
			return $this->load->view('payment/processingkz', $data);
		} else {			
    ...
    public function processing() {
        ...
        if ($this->config->get('processingkz_comission_status_id') == 'true') {
            ...
            $this->model_total_comission->getTotal($total_data, $total, $taxes);
        ...
        $transactionDetails->returnURL = HTTPS_SERVER . 'index.php?route=payment/processingkz/success&orderID=' . $this->session->data['order_id'] . '&sessionID=';
        ...
        if ($startTransactionResult->return->success == true) {
            ...
            $this->load->model('payment/processingkz');
            $vs = $this->model_payment_processingkz->addVisa($set);
            $this->model_payment_processingkz->addVisaToOrder($vs, $this->session->data['order_id']);
    ...
    public function success() {
        ...
        $this->load->model('payment/processingkz');
        $this->model_payment_processingkz->editVisa($set);
        $this->load->model('checkout/order');
