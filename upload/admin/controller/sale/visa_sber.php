...
class ControllerSaleVisaSber extends Controller {

    public function index() {
        
		$this->load->language('sale/visa_sber');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('sale/visa_sber', 'token=' . $this->session->data['token'], 'SSL')
		);
		...
		$visa_check = $this->visa_check();
        $data['visa_check'] = empty($visa_check) ? false : true;
		if (!empty($visa_check)) { 
		...
		$results = $this->model_sale_visa_sber->getOrders($param);
        ...
        foreach ($results as $result) {
            ...
            $action[] = array(
                ...
                'href' => $this->url->link('sale/visa_sber/info', 'token=' . $this->session->data['token'] .
        ...
        $this->response->setOutput($this->load->view('sale/visa_sber.tpl', $data));
    }
    ...
    public function refresh() {
        ...
        $results = $this->model_sale_visa_sber->getOrders($set);
        foreach ($results as $result) {
            $v = array(
                'order_id' => $result['order_id'],
                'customer_reference' => $result['customer_reference'],
        ...
        $status = $this->model_sale_visa_sber->editVisa($v);

        $this->language->load('sale/visa_sber');
    ...
    public function confirm() {
        ...
        $status = $this->model_sale_visa_sber->confirmVisa($v);
        if ($status != false) {
            $history = array(
                'notify' => '0',
                'order_status_id' => $this->config->get('processingkz_order_status_id'),
            );
            $this->model_sale_visa_sber->addOrderHistory($result['order_id'], $history);
    ...
    public function cancel() {
        ...
        $status = $this->model_sale_visa_sber->cancelVisa($v);
        if ($status != false) {
            $history = array(
                'notify' => '0',
    ...
    public function info() {
	   ...
	   $data['cancel'] = $this->url->link('sale/visa_sber', 'token=' . $this->session->data['token'], 'SSL');
        ...
        if ($status == 'AUTHORISED' or $status == 'PAID' or $status == 'DECLINED' or $status == 'REVERSED') {
            $results = $this->model_sale_visa_sber->getExtended($set);
        }
        ...
        $this->response->setOutput($this->load->view('sale/visa_sber_info.tpl', $data));       
    }
	
	public function visa_check() {
        $query = $this->db->query("SELECT * FROM information_schema.COLUMNS WHERE TABLE_NAME='" . DB_PREFIX . "order' AND COLUMN_NAME='visa_sber_id'");
        return $query->row;
    }

    public function visa_upgrade() {
        $sql = "CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "visa_sber` ( ";
        ...
        $sql = "ALTER TABLE `" . DB_PREFIX . "order` ADD COLUMN `visa_sber_id` INTEGER(11) NOT NULL;";
        $this->db->query($sql);
        return true;
    }
}
