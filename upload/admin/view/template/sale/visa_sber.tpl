...
<div id="content">
  <div class="page-header">
    <div class="container-fluid">
      ...
      <h1><img height="32" src="view/image/payment/smi_sber_logo_.jpg" alt="" /> <?php echo $heading_title; ?></h1>
      <ul class="breadcrumb">
  ...
  <div class="container-fluid">
    <div class="panel panel-default">     
		...
		<?php } else { ?>
                                <div class="jumbotron">
                                    <h1>Внимание!</h1>
                                    <p>Структура базы данных не соответствует версии модуля. Для приведения в соответствие нажмите кнопку ниже.</p>
                                    <p><a class="btn btn-primary btn-lg" role="button" OnClick="visaUpgrade();">Обновить базу данных</a></p>
                                </div>
                            <?php } ?>
        </div>
    </div>
...
function refresh(idd) { 
 //console.log(idd);
	$.ajax({
		...
		dataType: 'json',
		data: '&visa_id=' + idd,
		url: 'index.php?route=sale/visa_sber/refresh&token=' + token,
...
//console.log(id);
	$.ajax({
		...
		dataType: 'json',
		data: '&visa_id=' + id,
		url: 'index.php?route=sale/visa_sber/confirm&token=' + token,
			success: function(message) {
			document.all.s.rows[myRow].cells[3].innerHTML=message['status'] ;
            window.location.reload( true );
...
//console.log(id);
	$.ajax({
		...
		dataType: 'json',
		data: '&visa_id=' + id,
		url: 'index.php?route=sale/visa_sber/cancel&token=' + token,
			success: function(message) {
			document.all.s.rows[myRow].cells[3].innerHTML= message['status'] ;
			window.location.reload( true );
		//	document.all.s.rows[row].cells[6].innerHTML= '[ <a href="<?php echo $action['href']; ?>"><?php echo $action['text3']; ?></a> ]'+'[ <a onclick="refresh(<?php echo $order['visa_id']; ?>);"><?php echo $action['text']; ?></a> ]';
...
//console.log(id);
	$.ajax({
		...
		dataType: 'json',
		data: '&visa_id=' + id,
		url: 'index.php?route=sale/visa_sber/verified&token=' + token,
			success: function(message) {
